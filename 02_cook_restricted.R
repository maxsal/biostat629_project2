...
restricted <- read_tsv(here("project_2", "data", "restricted.txt"),
                       col_types = cols()) %>%
  mutate(
...
vip(best_rpart)
ggsave(here("project_2", "fig", "restricted_vip.pdf"), width = 7, height = 5)
...
roc_curves %>%
  ...
  geom_line(size = 1) +
  labs(
    title = "1-year restricted data"
  )
ggsave(here("project_2", "fig", "restricted_roc_plt.pdf"), width = 5, height = 5)
